...
public abstract class AbstractParser { //abstract donc on peut pa l'instancier
    public static Calendar chooseParser(String source, String type){ //static car sert de factory => fabrique l'instance, doit être accessible sans instance préalable
   ...
   //partie commune du parsing
    protected Calendar parseStream(Reader reader, String type ) throws IOException {
        ...
        try (IcsReader icsReader = new IcsReader(reader)) { //utilisation de la classe IcsReader pour gérer les lignes continues -> intéret de AutoCloseable
            String line;
    ...
    private Map<String, String> parseComponentProperties(IcsReader r, String componentName) throws IOException {
        ...
        while ((line = r.readLine()) != null) {
            ...
            if (parts.length == 2) {
                String key = parts[0].split(";")[0]; // On ignore les paramètres (ex: DTSTART;TZID=...) pour simplifier l'iter 1&2
                map.put(key, parts[1]);
            }
        }
        return map;
    ...
    protected Instant parseIcsDate(String s) {
        ...
        try {
            ...
            //pour une gestion complète des Timezones, c'est plus complexe, mais suffisant pour ce projet.
